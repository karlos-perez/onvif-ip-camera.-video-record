...
class ClientRTSP(Process):
    ...
    # TODO: how????
    # def _ring_buffer(self, queue, size):
        # buffer = deque()
        # start_time = time.time()
        # for i in self._get_frame():
        #     if time.time() - start_time < size:
        #         buffer.append(i)
        #     else:
        #         buffer.popleft()
        #         buffer.append(i)
    ...
    def _record(self, filename, durations=None):
    ...
    def run_record(self, durations=None, id_record=None):
        self._start()
        if id_record:
            filename =  self._filename(id_record)
        else:
            filename = self._filename()
        self._record(filename, durations)
        self._finish()
...
if __name__ == "__main__":
    ...
    # c.run_record(dur)
    stop_record = Event()
    proc = Process(target=c.run_record, args=(None, 1))
    proc.daemon = True
    proc.start()
